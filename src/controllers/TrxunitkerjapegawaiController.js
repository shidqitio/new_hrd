...
exports.index = (req, res, next) => {
    TrxUnitKerjaPegawai.findAll({ where: { kode_pegawai: req.params.kode_pegawai } })
        .then((app) => {
            if (!app) {
                const error = new Error("kode pegawai Tidak Terdaftar");
...
exports.store = (req, res, next) => {
    ...
    TrxUnitKerjaPegawai.findAll({ where: { kode_pegawai: req.body.kode_pegawai } })
        .then((app) => {
            if (app) {
                const error = new Error("kode pegawai Sudah Terdaftar");
                error.statusCode = 422;
            }
            return db.transaction()
                .then((t) => {
                    TrxUnitKerjaPegawai.findOne({ where: { kode_pegawai: req.body.kode_pegawai } })
                        .then((app) => {
                            if (app) {
                                const error = new Error("Kode pegawai Sudah Terdaftar");
                                error.statusCode = 422;
